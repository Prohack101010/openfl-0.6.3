...
class DisplayObjectRenderer extends EventDispatcher
{
	...
	@:noCompletion private var __pixelRatio:Int;
	...
	@:noCompletion private function __updateCacheBitmap(displayObject:DisplayObject, force:Bool):Bool
	{
		...
		if (displayObject.cacheAsBitmap || (renderer.__type != OPENGL && !colorTransform.__isDefault(true)))
		{
			...
			#if openfl_disable_cacheasbitmap_highdpi
			var pixelRatio = 1;
			#else
			...
			if (updateTransform || needRender)
			{
				...
				offsetX = rect.x > 0 ? Math.ceil(rect.x) * pixelRatio : Math.floor(rect.x) * pixelRatio;
				offsetY = rect.y > 0 ? Math.ceil(rect.y) * pixelRatio : Math.floor(rect.y) * pixelRatio;
